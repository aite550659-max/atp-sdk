...
|---------|--------|----------|-------------|
| `val`   | string | yes      | Protocol version. Always `"1.0"` for this spec. |
...
async function createAgentLog(name, soulHash, capabilities) {
  ...
  const attestation = JSON.stringify({
    val: "1.0", type: "agent.create", ts: new Date().toISOString(),
    agent: topicId,
    data: { name, soul_hash: soulHash, capabilities, framework: "val-ref/1.0" }
  });
...
async function attestAction(topicId, tool, inputData, outputData, desc) {
  const attestation = JSON.stringify({
    val: "1.0", type: "action", ts: new Date().toISOString(),
    agent: topicId,
    data: {
