  ...
  try {
    switch (url.pathname) {
      ...
      case "/v1/swap/create": {
        ...
        const { from: swapFrom, amount: swapAmount, accountId: swapAccount, memo, payoutMemo } = body;
        if (!swapFrom || !swapAmount || !swapAccount) {
          result = { status: 400, body: { error: "from, amount, and accountId required" } };
        } else {
          const swap = await createSwap(swapFrom, swapAmount, swapAccount, payoutMemo || memo || "");
