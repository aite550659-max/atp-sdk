 ...
 */
export async function createSwap(fromToken, amount, hederaAccountId, payoutMemo = "") {
  const from = resolveTicker(fromToken);
  ...
  return {
    ...
    payoutMemo: payoutMemo || null,
    status: "waiting",
    message: `Send ${amount} ${fromToken.toUpperCase()} to ${data.payinAddress}${data.payinExtraId ? ` (memo: ${data.payinExtraId})` : ""}. HBAR will arrive in ${hederaAccountId}${payoutMemo ? ` with memo ${payoutMemo}` : ""} within 10-60 minutes.`,
  };
}
